...
//   return (
//     <Badge variant="outline" className={`text-[10px] font-bold gap-1.5 rounded-[8px] uppercase tracking-wide ${s.badge}`}>
...
//       <Badge variant="outline" className={`text-[10px] font-bold w-fit rounded-[8px] uppercase tracking-wide ${DP_STYLE[status]}`}>
//         {status}
//       </Badge>
//       {!!plant && <span className="text-[10px] text-pt-neutral-500 font-mono pl-0.5">{plant}</span>}
//     </div>
//   );
// }
...
//         <div className={`${ORDER_STYLE[status].bar} h-full rounded-full`} style={{ width: `${pct}%` }} />
//       </div>
...
//           >
//             <span className="font-mono font-medium text-pt-neutral-900 w-24 shrink-0">{d.date}</span>
//             <span className="font-mono text-pt-neutral-500 w-10 shrink-0">{d.time}</span>
//             <span className="text-pt-neutral-700 flex-1">{d.note}</span>
//             <span className="text-pt-neutral-500 text-[11px] w-16 text-right shrink-0">{d.truck}</span>
...
//         <span>ส่งแล้ว <strong className="text-pt-primary-600 font-mono">{sent.toLocaleString()}</strong> คิว</span>
//         <span>คงเหลือ <strong className={`font-mono ${rem === 0 ? "text-pt-success-600" : "text-pt-warning-600"}`}>{rem.toLocaleString()}</strong> คิว</span>
//         <span>ความคืบหน้า <strong className="font-mono text-pt-neutral-900">{progress(sent, order.totalQty)}%</strong></span>
//       </div>
//     </div>
//   );
// }
...
//           {showCustomer ? (
//             <>
//               <p className="font-semibold text-[13px] text-pt-neutral-900 leading-tight">{custName}</p>
...
//           >
//             {site.siteCode}
//             {order.deliveries.length > 0 && (open ? <ChevronDown size={11} /> : <ChevronRight size={11} />)}
//           </button>
...
//           <span className="font-mono font-semibold text-[13px] text-pt-neutral-700">{order.totalQty.toLocaleString()}</span>
//           <p className="text-[10px] text-pt-neutral-500">คิว</p>
//         </TableCell>

//         <TableCell className="text-right w-[90px]">
//           <span className={`font-mono font-semibold text-[13px] ${sent > 0 ? "text-pt-primary-600" : "text-pt-neutral-400"}`}>
//             {sent.toLocaleString()}
//           </span>
...
//           <p className="text-[12px] font-medium text-pt-neutral-700">{order.schedDate || "—"}</p>
//           {!!order.schedTime && <p className="text-[11px] font-mono text-pt-neutral-500">{order.schedTime}</p>}
//           {!!order.truck && order.truck !== "-" && <p className="text-[11px] text-pt-neutral-500 mt-0.5">🚛 {order.truck}</p>}
//         </TableCell>
...
//           open ? "bg-pt-primary-50 border-pt-primary-300" : "bg-pt-neutral-50 border-pt-neutral-200 hover:bg-pt-neutral-100"
//         }`}
//         onClick={() => setOpen((v) => !v)}
//       >
//         <TableCell colSpan={10} className="py-2.5 px-4">
//           <div className="flex items-center gap-3 flex-wrap">
//             <span className={`w-5 h-5 rounded-[8px] flex items-center justify-center shrink-0 ${open ? "bg-pt-primary-600 text-white" : "bg-pt-neutral-200 text-pt-neutral-500"}`}>
//               {open ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
//             </span>

//             <div className="flex-1 min-w-0">
//               <span className="font-bold text-[13px] text-pt-neutral-900">{customer.companyName}</span>
//               <span className="text-[11px] text-pt-neutral-500 ml-2">
//                 {customer.id} · {customer.phone} · Line: {customer.contactLine}
//               </span>
//             </div>

//             <Badge variant="outline" className="text-[10px] rounded-[8px] bg-white border-pt-neutral-300 text-pt-neutral-600">
//               {customer.sites.length} Site · {allOrders.length} Order
//             </Badge>

//             <Badge variant="outline" className="text-[10px] rounded-[8px] bg-pt-success-100 border-pt-success-300 text-pt-success-700">
...
//     >
//       <p className="text-[10px] font-bold text-pt-neutral-500 uppercase tracking-widest mb-1.5">{label}</p>
//       <div className="flex items-baseline gap-2">
//         <span className="text-[26px] font-bold text-pt-neutral-900 leading-none font-mono">{value}</span>
...
//       <label className="text-[10px] font-semibold text-pt-neutral-500 uppercase tracking-wider">{label}</label>
//       {children}
//     </div>
//   );
// }
...
//   const allData = DashboardService.getDashboardData();
...
//   const [dateTimeNow, setDateTimeNow] = useState(formatNowTH(new Date()));

//   useEffect(() => {
//     const timer = setInterval(() => setDateTimeNow(formatNowTH(new Date())), 1000);
...
//               ดึงข้อมูลจาก QO / Queue ที่ map จาก DB จริง อัปเดตล่าสุด {dateTimeNow}
//             </p>
//           </div>
...
//           <StatCard label="จำนวนคำสั่งจอง (Orders)" value={String(totalOrders)} sub="รายการ" accentClass="border-l-pt-primary-600" />
//           <StatCard label="จำนวน Site code ที่เปิด" value={String(totalSites)} sub="ไซต์" accentClass="border-l-pt-secondary-dark-accent" />
//           <StatCard
//             label="คำสั่งที่ต้องเร่งด่วน/ติดตาม"
//             value={String(allOrders.filter((o) => o.status === "อยู่ระหว่างจัดส่ง").length)}
//             sub="รายการ"
//             accentClass="border-l-pt-warning-500"
//           />
//           <StatCard
//             label="คำสั่งที่มีประเด็นหรือปัญหา"
//             value={String(allOrders.filter((o) => o.status === "ยกเลิกรายการ").length)}
//             sub="จุด"
//             accentClass="border-l-pt-error-600"
//           />
//           <StatCard label="มูลค่าคำสั่งซื้อรวม (วันนี้)" value="-" sub="รอ API ราคา" accentClass="border-l-pt-success-600" />
//           <StatCard label="มาร์จิ้นคำสั่งซื้อรวม (วันนี้)" value="-" sub="รอ API margin" accentClass="border-l-pt-neutral-700" />
//         </div>
//       </div>
...
//               <SelectTrigger className="h-9 text-[12px] w-44 rounded-[16px] border-pt-neutral-300" style={{ boxShadow: "var(--pt-shadow-xs)" }}>
...
//               <SelectTrigger className="h-9 text-[12px] w-40 rounded-[16px] border-pt-neutral-300" style={{ boxShadow: "var(--pt-shadow-xs)" }}>
...
//               {filtered.length > 0 && <span className="ml-2 text-pt-neutral-400">(หน้า {page} จาก {totalPages})</span>}
//             </p>
//             <p className="text-[10px] text-pt-neutral-400">▸ คลิก Site chip เพื่อดูประวัติการจัดส่ง</p>
//           </div>
...
//               <TableBody>
//                 {paginated.length > 0 ? (
//                   paginated.map((c) => <CustomerGroupRows key={c.id} customer={c} />)
//                 ) : (
//                   <TableRow>
...
//                 <strong className="text-pt-neutral-700">{totalPages}</strong> · {filtered.length} รายการทั้งหมด
//               </p>
